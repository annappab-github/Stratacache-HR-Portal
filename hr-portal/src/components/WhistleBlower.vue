          ...
          <div class="emailLoader" v-else-if="emailLoading"></div>
            <div class="col-sm-3">
                <div class="text-center faq">
                    <img  :src="policydocimg" class="round" alt="" @click="openDocument()">
                    <div class="font-weight-bold text-center support-text-title">WHISTLE BLOWER</div>
                    <div class="text-center  support-text-body">Policy document.</div>
                </div>
            </div>
            <div class="col-sm-3 reportIssue">
                <div class="text-center" @click="showEmailModal('Staging')">
                    <img :src="whistleimg" class="round" alt="" >
                    <div class="font-weight-bold text-center support-text-title">EMAIL ANONYMOUSYLY</div>
                    <div class="text-center support-text-body">Whistle blower email.</div>
                </div>
            </div>
        </div>
    <div class="d-flex justify-content-sm-center">
        <div class="forgot-password-text" @click=" $emit('openWhistleBlower',false)">
           <img :src="loginImg" class="loginImg" alt="">
           <label class="ml-2" >Back</label>
        </div>
        </div>
    </div>
...
<div id="overlay">
  <div id="text">
      ...
      <div class="mb-1 mt-2 row">
        ...
        <input type="text" id="form3Example4cg" v-model="form.subject"  placeholder="Please enter subject" class="text-line otherSubField" autocomplete="off"/>
      </div>
      <hr>
        <div class="">
...
export default {
  ...
  data() {
    return {
    form: {
        to: process.env.VUE_APP_MD_EMAIL,
        subject:'',
        message: '',
        from: process.env.VUE_APP_ANONYMOUS_EMAIL,
        type: ''
      },
      loading: false,
      emailLoading: false,
 ...
 mounted(){
   ...
   methods: {
    ...
    openDocument() {
      ...
      this.active =  true;
      this.pdfView =  true;
      this.pdfFile = '../assets/handbook.pdf';
      this.currentPage = 1;
      var self = this;
    ...
    sendEmail() {
      ...
      // this.form.from = this.loggedInUser[10];
