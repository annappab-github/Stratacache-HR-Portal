...
class ConsolidatedFlightDataResource extends JsonResource
{
    ...
    public function toArray($request)
    {
      ...
      if ($this->codeshares->count() > 0) {
        if (isset($this->codeshares->code) ) array_push($result["numbers"], $this->codeshares->code);
    ...
    private function formatTime($time) {
      return isset($time) ? (new DateTime($time))->setTimezone(new DateTimeZone(Config::get('app.clientTimeZone')))->format('Y-m-d H:i:s'): null;
    }
}
